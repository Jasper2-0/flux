...
def do_imp(dll,nm):
    ...
    if nm=='timeGetTime': return ret_with(0,0)
...
def h_texfetch(uc,addr,size,ud):
    esi=uc.reg_read(UC_X86_REG_ESI)
    try: op=uc.mem_read(esi,1)[0]
    except Exception: op=-1
    texprog.append((esi,op))
uc.hook_add(UC_HOOK_CODE,h_texfetch,begin=0x4418b7,end=0x4418b7)
...
print('script range: %#x..%#x'%(min(o['p'] for o in ops),max(o['p'] for o in ops)))
print('TEXTURE allocs:',len(allocs),sorted(set(allocs)))
print('TEXTURE programs:',len(texentry),[hex(x) for x in texentry])
print('TEXTURE opcode fetches:',len(texprog))
used=sorted({o for _,o in texprog})
print('OPERATORS USED (full):',[hex(u) for u in used],'count',len(used))
import json
json.dump({'ops':ops,'toks':toks},open('script_dump_full.json','w'))
json.dump({'entry':texentry,'prog':texprog,'allocs':allocs},open('texprog_full.json','w'))
print('wrote script_dump_full.json + texprog_full.json')
...
print('last blocks:',[hex(a) for a in tail])
