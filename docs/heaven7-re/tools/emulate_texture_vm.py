...
texprog=[]      # (esi_before, opcode)
texentry=[]     # program pointers passed to the interpreter
def h_texvm_entry(uc,addr,size,ud):
    texentry.append(uc.reg_read(UC_X86_REG_EDI))
uc.hook_add(UC_HOOK_CODE,h_texvm_entry,begin=0x44189f,end=0x44189f)
def h_texvm_fetch(uc,addr,size,ud):
    esi=uc.reg_read(UC_X86_REG_ESI)
    try: op=uc.mem_read(esi,1)[0]
    except Exception: op=-1
    texprog.append((esi,op))
    if len(texprog)>=6000: uc.emu_stop()
uc.hook_add(UC_HOOK_CODE,h_texvm_fetch,begin=0x4418b7,end=0x4418b7)
...
try:
    uc.emu_start(ENTRY,0,count=400_000_000)
except UcError as e:
...
print('last blocks:',[hex(a) for a in tail][-5:])
print('texture-VM invocations:',len(texentry),'program ptrs:',[hex(x) for x in texentry[:8]])
print('texture-VM opcode fetches:',len(texprog))
print('first ops:',[(hex(p),hex(o)) for p,o in texprog[:24]])
import json; json.dump({'entry':texentry,'prog':texprog},open('texprog_dump.json','w'))
